...
';
	...
	public function validate_options( $input ) {
		return str_replace( '-', '_', sanitize_title_with_dashes( $input ) );
	}
	/**
	 * Outputs theme zip.
	 */
	public function export_callback() {
		...
		//$child_slug = strtolower( preg_replace('/[^a-zA-Z0-9-_\.]/', '', $child_name ) );
		//$child_slug = str_replace( '-', '_', sanitize_title_with_dashes( $child_name ) );
		$child_theme_slug = kadence_build_child_get_option( 'kadence_build_child_theme_config', 'kct_slug', '' );
		if ( ! empty( $child_theme_slug ) ) {
			$child_slug = $this->slugify( $child_theme_slug );
		} else {
			$child_slug = str_replace( '-', '_', $this->slugify( $child_name ) );
		}
		$child_folder = $child_slug . '/';
		//$file_path = KADENCE_BUILD_CHILD_PATH . '/temp/' . $child_slug . '/';
		$zip = new ZipArchive();
		$filename = $child_slug . '.zip';
