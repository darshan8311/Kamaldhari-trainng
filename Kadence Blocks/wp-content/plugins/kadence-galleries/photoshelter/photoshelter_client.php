<?php
	...
	//	echo "<pre>" . print_r($a) . "</pre>";
		//exit();
	//}
// Pulled from offical photoshelter plugin.

class KT_Photoshelter_Client {
	...
	function auth() {
		...
		$authUrl = KT_Photoshelter_Client::BASE_URL . 'authenticate?email=' . $this->good_encode($options['username']) . "&password=" . $this->good_encode($options['password']);
		$sessUrl = KT_Photoshelter_Client::BASE_URL . 'user/session';
	...
	function org_auth($O_ID) {
		...
		$url = KT_Photoshelter_Client::BASE_URL . 'organization/' . $O_ID . '/authenticate';
	...
	function idle(){
		$url = KT_Photoshelter_Client::BASE_URL . 'idle';
		$data = $this->make_request('GET', $url);
	...
	function gal_qry($page_url=null, $sort_by=null, $sort_dir="asc"){
		...
		$url = $page_url ? $page_url :  KT_Photoshelter_Client::BASE_URL . 'gallery?&ppg='. $ppg . '&page=1' . $sort;
		$data = $this->make_request('GET', $url);
		$galleries = array();
	...
	function gal_getVis($g_id) {
		$url = KT_Photoshelter_Client::BASE_URL . 'gallery/' . $g_id . '/visibility';
		$data = $this->make_request('GET', $url);
	...
	function get_custom_url() {
		$url = KT_Photoshelter_Client::BASE_URL . 'user/settings/site-url';
		$data = $this->make_request('GET', $url);
	...
	function img_qry($g_id = ''){
		...
		if (!empty($_GET['page_url'])) {
			...
			$url = KT_Photoshelter_Client::BASE_URL . 'image/query';
			if(!empty($g_id)) {
	...
	function img_get($i_id) {
		$url  = KT_Photoshelter_Client::BASE_URL . 'image/' . $i_id. '?mcSet=t';
		$data = $this->make_request('GET', $url);
	...
	function img_is_public($i_id) {
		$url = KT_Photoshelter_Client::BASE_URL . 'image/' . $i_id. '/public';
		$data = $this->make_request('GET', $url, false);
		return ($data->response->data->is_public == 't');
	}

	function img_searchable($i_id) {
		$url  = KT_Photoshelter_Client::BASE_URL . 'image/' . $i_id. '/update?f_searchable=t';
		$data = $this->make_request('GET', $url);
		$img  = (array) $data->response->data;
		return $img;
	}
	...
	function img_widget_get($i_id) {
		$url = KT_Photoshelter_Client::BASE . '/rss/imgWidget/' .$i_id. '?mcSet=t';
	...
	function make_request($method, $url, $use_cookie = true, $type='json') {
		...
		//check for session
		if (isset($return->response->error) && (string) $return->response->error->class == 'SessionRequiredErr') {
			$this->logged_in = false;
	...
	function ss_preset_qry() {
		$url = KT_Photoshelter_Client::BASE_URL . 'user/settings/slideshow/query';
	...
	function ss_preset_get($d_id, $g_id) {
		$url = KT_Photoshelter_Client::BASE_URL . 'user/settings/slideshow/' . $d_id . '?gallery_id=' . $g_id;
	...
	function img_search($term = null, $page_url = null, $sort_by='date', $sort_dir='dsc') {
		$ppg = 100;
		$url = KT_Photoshelter_Client::BASE_URL . '/image/search?sort_by='.$sort_by.'&sort_dir='.$sort_dir;
...
$GLOBALS['psc'] = new KT_Photoshelter_Client();
?>
